...
def compute_symbol_stats(symbol):
    ...
    per_day_all = df.groupby("date").size().sort_index()

    # Exclude real exchange holidays from the trading-day count — a holiday
    # currently gets logged all day with frozen, repeated prices and would
    # otherwise be wrongly counted as a valid day (see Modification List
    # item 22). Kept separately so the excluded days are visible, not just
    # silently dropped.
    holiday_days = [d for d in per_day_all.index if holidays.is_holiday(symbol, d.strftime("%Y-%m-%d"))]
    per_day = per_day_all.drop(index=holiday_days)

    return {
        "trading_days": len(per_day),
        "total_rows": len(df),
        "date_min": per_day.index.min() if len(per_day) else None,
        "date_max": per_day.index.max() if len(per_day) else None,
        "per_day": per_day,
        "holiday_days_excluded": holiday_days,
...
def report_for_symbol(symbol):
    ...
    if len(stats["holiday_days_excluded"]) > 0:
        log.info(f"\nExcluded {len(stats['holiday_days_excluded'])} real exchange holiday(s) from the count: "
                  f"{', '.join(str(d) for d in stats['holiday_days_excluded'])}")
